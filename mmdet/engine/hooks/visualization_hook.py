    ...
    def after_test_iter(self, runner: Runner, batch_idx: int, data_batch: dict,
        ...
        for data_sample in outputs:
            ...
            if isinstance(text, str):  # VG
                ...
                elif gt_instances.get('labels', None) is not None:
                    gt_labels = gt_instances.labels
                else:
                    num_gt = len(gt_bboxes) if gt_bboxes is not None else 0
                    if tokens_positive == -1:
                        gt_labels = np.zeros(num_gt, dtype=np.int64)
                    else:
                        gt_labels = np.arange(num_gt, dtype=np.int64)
                ...
                if tokens_positive == -1:  # REC
                    ...
                    for label in gt_labels:
                        ...
                        if label < len(tokens_positive):
                            gt_tokens_positive.append(tokens_positive[label])
