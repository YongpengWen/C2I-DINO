    ...
    def assign(self,
        ...
        for match_cost in self.match_costs:
            ...
            if not torch.isfinite(cost).all():
                ...
                bad_count = int((~torch.isfinite(cost)).sum().item())
                img_id = None if img_meta is None else img_meta.get('img_id')
                img_path = None if img_meta is None else img_meta.get('img_path')
                logger.warning(
                    'Non-finite Hungarian cost detected in %s: '
                    'bad_count=%d, img_id=%s, img_path=%s. '
                    'Replacing non-finite entries with large finite costs.',
                    match_cost.__class__.__name__, bad_count, img_id, img_path)
        ...
        if not torch.isfinite(cost).all():
            ...
            img_path = None if img_meta is None else img_meta.get('img_path')
            logger.warning(
                'Non-finite combined Hungarian cost detected: '
                'bad_count=%d, img_id=%s, img_path=%s. '
                'Replacing non-finite entries with large finite costs.',
                bad_count, img_id, img_path)
