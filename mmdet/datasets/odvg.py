    ...
    def load_data_list(self) -> List[dict]:
        ...
        for data in data_list:
            ...
            if self.dataset_mode == 'OD':
                ...
                if 'target_class' in anno:
                    data_info['target_class'] = anno['target_class']
                if 'caption_suffix' in anno:
                    data_info['caption_suffix'] = anno['caption_suffix']
                regions = anno['regions']
                if regions and 'target_phrase' in regions[0]:
                    data_info['target_phrase'] = regions[0]['target_phrase']
                ...
                for i, region in enumerate(regions):
                    ...
                    for box in bbox:
                        ...
                        if 'target_class' in region:
                            phrases[i]['target_class'] = region[
                                'target_class']
                        if 'target_phrase' in region:
                            phrases[i]['target_phrase'] = region[
                                'target_phrase']
                        if 'target_tokens_positive' in region:
                            phrases[i]['target_tokens_positive'] = region[
                                'target_tokens_positive']
                        if 'caption_suffix' in region:
                            phrases[i]['caption_suffix'] = region[
                                'caption_suffix']
                        instances.append(instance)
                data_info['instances'] = instances
